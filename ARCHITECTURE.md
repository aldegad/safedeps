...
> *It approves one dependency closure from provider evidence first, then the post hook treats the installed lockfile closure as the authority; reorg rolls back any unapproved or newly vulnerable effect.*

번역: Safedeps 는 install 순간에 여러 라이브 truth 를 동시에 조회해서 결정하지 않는다. **사전에 provider evidence 로 안전한 dependency closure 를 먼저 승인** (approve) 하고, **PostToolUse hook 은 실제 lockfile closure 를 ledger 와 대조**한다. 미승인 또는 새 취약 effect 가 있으면 **reorg 가 rollback layer** 로 동작한다. npm 은 OSV `/v1/querybatch` + 24h pkg@version cache 로 direct/transitive 전체를 검증한다. 다른 ecosystem 의 closure 해석은 후속.
...
│   ─────────────────────                 ──────────────────            │
│   Phase 1  closure advisory gate        safedeps scan secrets         │
│            (npm: OSV batch + KEV)           (gitleaks worktree/staged)  │
│   Phase 2  fast command gate            safedeps audit deps           │
│            (PreToolUse ledger UX)          (npm audit / pip-audit)     │
│   Phase 3  effect enforcement           safedeps hooks install|check  │
│            (PostToolUse closure→ledger)    (repo-local git hook)       │
...
```
   ...
   │                  ┌────────┴────────┐                                │
   │                  ▼                 ▼                                │
   │              closure 승인됨    unapproved/vulnerable                 │
   │                  │                 │                                │
   │                  ▼                 ▼                                │
   │              CONFIRM            REORG                               │
...
핵심 통찰:
- **Phase 1 = pre-install closure advisory gate**. npm 은 temp dir 에서 `npm install <pkg>@<version> --package-lock-only --ignore-scripts` 로 lockfile 을 만들고 full closure 를 추출한다. OSV `/v1/querybatch` 로 direct/transitive 전체를 묶어 조회하고, clean 일 때 direct ledger entry 에 `transitive_specs` 를 기록한다.
- **Phase 2 = fast command gate**. PreToolUse 명령 파싱은 빠른 UX 차단과 snapshot 준비 역할이다. 우회/오탐을 줄이는 advisory layer 이며, 최종 권위는 설치 결과를 보는 post gate 다.
- **Phase 3 = effect enforcement + rollback**. PostToolUse 가 실제 `package-lock.json` closure 전체를 ledger direct entry + `transitive_specs` 와 대조한다. 미승인 또는 OSV/KEV 취약 closure 가 있으면 마지막 confirmed 로 reorg 한다.
...
- `transitive_specs` — direct ledger entry 가 승인한 전체 transitive closure. PostToolUse effect gate 는 direct entry 또는 이 배열에 없는 `pkg@version` 을 미승인 effect 로 보고 reorg 한다.
...
- 같은 OS 사용자 권한으로 `~/.safedeps/approved-specs/` 를 직접 작성/수정하는 공격. 현재 ledger 는 로컬 convenience cache 이며, 서명/HMAC 또는 install-time 재조회가 도입되기 전에는 same-user 공격의 보안 경계가 아니다.
