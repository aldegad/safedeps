...
**A run judged against a moved source says so.** Pointing `SAFEDEPS_OSV_API_URL` (or the KEV/GHSA URLs, the closure fixtures, or a non-default ledger TTL) somewhere else is a real need — a mirror on a network that blocks osv.dev, a fixture in the test suite — so none of it is refused. What would be wrong is a run that answered from a moved truth looking exactly like a run that answered from OSV. Each deviation is recorded once per run, named, in `advisory.log`.
