...
- npm 패키지 version 은 `package.json` 이 SSoT. `bin/safedeps` `SAFEDEPS_VERSION` 이 이를 따라가고, smoke 테스트는 `package.json` 을 읽어 대조한다 (현재 v2.8.1).
...
- **pnpm / yarn lockfile audit** — `safedeps audit` 는 `package-lock.json` / `npm-shrinkwrap.json` 만 읽어서, pnpm(`pnpm-lock.yaml`)·yarn(`yarn.lock`) 프로젝트는 exit 2(판정 없음)가 나고 pre-commit dep audit 가 커버하지 못한다. lockfile 파서를 추가해 그 워크스페이스도 audit 한다.
