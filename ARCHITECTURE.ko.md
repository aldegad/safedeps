...
이 경우는 이제 생태계와 명령을 담은 `UNGATED` 기록을 남긴다. 판정은 하나도 안 바뀐다. 버전 없는 설치를 전부 거부하는 건 평범한 작업 흐름을 깨는 정책 변경이라 레포 소유자 소관으로 남기고, 기록은 그 결정을 추측이 아니라 근거로 답할 수 있게 만드는 것이다. 기록의 침묵도 소음만큼 의도적으로 범위를 잡았다 — 파일 기반 설치와 맨 lockfile 설치는 패키지를 지목하지 않아 빠진다. 평범한 설치마다 찍히는 기록은 배경 소음이고, 배경 소음은 없는 기록과 같기 때문이다.
